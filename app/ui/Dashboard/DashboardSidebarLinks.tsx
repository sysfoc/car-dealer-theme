...
export default function DashboardSidebarLinks({
  ...
  useEffect(() => {
    const activeLink = DashboardNavLinks.find(
      (item) =>
        item.link === currentPath ||
        item.subLinks?.some((sub) => `/dashboard${sub.link}` === currentPath)
    );

    if (activeLink) {
      setBreadcrumbLinks([
        { title: "Dashboard", link: "/dashboard" },
        { title: activeLink.title, link: currentPath },
      ]);
    } else {
      setBreadcrumbLinks([{ title: "Dashboard", link: "/dashboard" }]);
    }
  }, [currentPath, setBreadcrumbLinks]);
          ...
          {DashboardNavLinks.map((element, index) => (
            <li key={index} className="mb-2">
              {element.subLinks ? (
                <>
                  <button
                    onClick={() => toggleDropdown(element.title)}
                        ...
                        <li key={subIndex}>
                          <Link
                            ...
                            className={`block px-3 py-1 hover:bg-gray-200 text-sm ${
                              currentPath.includes(subLink.link)
